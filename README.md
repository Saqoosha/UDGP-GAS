...
├── KVS.ts                 # 設定値管理
├── Constants.ts           # 定数定義
├── DataModels.ts          # TypeScript型定義
├── SheetService.ts        # シートアクセスサービス
├── HeatGenerator.ts       # ヒート生成アルゴリズム
...
clasp tail              # リアルタイムログ表示
clasp deployments       # デプロイメント一覧
```
...
### TypeScriptエラー
- `SheetService is not defined`エラーの場合、`npm run build`でバンドルを再生成

## 最近の更新

- **パフォーマンス改善**: ボーダー追加をデータ挿入時にインライン化
- **時刻表示**: 完全な日時から時刻のみ（h:mm:ss）に変更
- **0ラップ対応**: ラップ数0のパイロットを正しく処理
- **TypeScriptバンドル**: ファイル読み込み順序問題を解決
